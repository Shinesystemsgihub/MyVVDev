...
if ( ! class_exists( 'MVVPS_Abstract_Setting' ) ) {
	/**
	 * Class MVVPS_Abstract_Setting.
	 */
	abstract class MVVPS_Abstract_Setting {
		/**
		...
		public function multiple_checkbox_callback( $args ) {
			include MVVPS_DIR . 'includes/admin/views/html-multiple-checkbox-setting.php';
		}
		...
		public function select_callback( $args ) {
			...
			<div class="mvvps-notes"><?php echo $args['desc']; ?></div>
			<?php
		}

		public function reset_counter_callback( $args ) {
			$class = ( isset( $args['class'] ) ) ? $args['class'] : "mvvps-notes";
			?>
			...
			<input id="<?php echo $args['id']; ?>"
			       ...
			       value="1"
				<?php
				checked( (bool) get_transient( 'mvvps_next_invoice_number' ) );
		...
		public function next_invoice_number_callback( $args ) {
			$class               = ( isset( $args['class'] ) ) ? $args['class'] : "mvvps-notes";
			$next_invoice_number = get_transient( 'mvvps_next_invoice_number' );
			?>
			<input id="<?php echo $args['id']; ?>"
			       name="<?php echo $args['page'] . '[' . $args['name'] . ']'; ?>"
			       type="<?php echo $args['type']; ?>"
			       value="<?php echo esc_attr( ( false !== $next_invoice_number ) ? $next_invoice_number : MVVPS_Abstract_Invoice::get_max_invoice_number() + 1 ); ?>"
				<?php
				if ( isset ( $args['attrs'] ) ) {
					foreach ( $args['attrs'] as $attr ) {
						echo $attr . ' ';
