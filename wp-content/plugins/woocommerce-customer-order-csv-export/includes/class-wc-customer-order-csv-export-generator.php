...
class WC_Customer_Order_CSV_Export_Generator {
	...
	private function get_orders_csv_row_data( $order_id ) {
		...
		$order_data = array(
			...
			'order_date'             => SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ? $order->get_date_created()->date( 'Y-m-d H:i:s' ) : $order->order_date,
	...
	private function get_customers_csv_row_data( $id, $order_id = null ) {
		...
		if ( ! $user && is_numeric( $order_id ) ) {
			...
			if ( $order ) {
				...
				$user->user_pass           = '';
				$user->user_registered     = SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ? $order->get_date_created()->date( 'Y-m-d H:i:s' ) : $order->order_date;
