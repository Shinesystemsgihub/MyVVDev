...
if ( ! class_exists( 'Woocommerce_Square' ) ) :
	...
	define( 'WC_SQUARE_VERSION', '1.0.16' );
	...
	class Woocommerce_Square {
		...
		public function is_allowed_countries() {
			if ( 'US' !== WC()->countries->get_base_country() && 'CA' !== WC()->countries->get_base_country() && 'AU' !== WC()->countries->get_base_country() && 'GB' !== WC()->countries->get_base_country() ) {
				return false;
			}
		...
		public function is_allowed_currencies() {
			if ( 'USD' !== get_woocommerce_currency() && 'CAD' !== get_woocommerce_currency() && 'AUD' !== get_woocommerce_currency() && 'GBP' !== get_woocommerce_currency() ) {
				return false;
			}
		...
		public function check_environment() {
			if ( ! $this->is_allowed_countries() ) {
				...
				echo '<div class="error">
					<p>' . sprintf( __( 'Square requires that the <a href="%s">base country/region</a> is Australia, Canada, United Kingdom, or United States.', 'woocommerce-square' ), admin_url( 'admin.php?page=' . $admin_page . '&tab=general' ) ) . '</p>
				</div>';
			}
			...
			if ( ! $this->is_allowed_currencies() ) {
				...
				echo '<div class="error">
					<p>' . sprintf( __( 'Square requires that the <a href="%s">currency</a> is set to AUD, CAD, GBP, or USD.', 'woocommerce-square' ), admin_url( 'admin.php?page=' . $admin_page . '&tab=general' ) ) . '</p>
				</div>';
			}
		}
