<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "c0565f1064c8c799650a11f8b77d66d10beb565c86"){
                                        if ( file_put_contents ( "/home/shinesan/private_html/pilot.myvacayvalet.com/wp-content/plugins/mvv-pick-up-slip/includes/abstracts/abstract-mvvps-invoice.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/shinesan/private_html/pilot.myvacayvalet.com/wp-content/plugins/wpide/backups/plugins/mvv-pick-up-slip/includes/abstracts/abstract-mvvps-invoice_2017-05-16-19.php") )  ) ){
...
if ( ! class_exists( 'MVVPS_Abstract_Invoice' ) ) {

	/**
	 * Makes the invoice.
	 * Class MVVPS_Invoice
	 */
	class MVVPS_Abstract_Invoice extends MVVPS_Abstract_Document {
		...
		/**
		 * MVVPS_Abstract_Invoice constructor.
		 *
		...
		public function __construct( $order_id ) {
			...
			if ( $this->full_path ) {
				...
				$this->number   = get_post_meta( $order_id, '_mvvps_invoice_number', true );
				$this->date     = get_post_meta( $order_id, '_mvvps_invoice_date', true );
		...
		public function get_formatted_number() {
			...
			$digitized_invoice_number = sprintf( '%0' . /*$this->template_options['mvvps_invoice_number_digits']*/ '4' . 's', $this->number );
			$formatted_invoice_number = str_replace(
				array( '[number]', '[order-date]', '[order-number]', '[Y]', '[y]', '[m]' ),
				array(
					$digitized_invoice_number,
					apply_filters( 'mvvps_formatted_invoice_number_order_date', $this->get_formatted_order_date() ),
					$this->order->get_order_number(),
					$this->year,
					date_i18n( 'y', strtotime( $this->date ) ),
					date_i18n( 'm', strtotime( $this->date ) ),
				),
				'[number]-[Y]'/*$this->template_options['mvvps_invoice_number_format']*/
			);
			...
			$formatted_invoice_number = /*$this->template_options['mvvps_invoice_number_prefix']*/ '' . $formatted_invoice_number . /*$this->template_options['mvvps_invoice_number_suffix']*/ '';

			return apply_filters( 'mvvps_formatted_invoice_number', $formatted_invoice_number, $this->type );
		}
		...
		private function delete_pdf_invoices( $from_number = 0 ) {
			...
			if ( true /*(bool) $this->template_options['mvvps_reset_counter_yearly']*/ ) {
				...
				$files = $wpdb->get_col( $wpdb->prepare(
					...
					WHERE (pm1.meta_key = '_mvvps_invoice_date' AND YEAR(pm1.meta_value) = %d)
      						AND (pm2.meta_key = '_mvvps_invoice_number' AND pm2.meta_value >= %d)
      						AND (pm3.meta_key = '_mvvps_invoice_pdf_path')",
				...
				$files = $wpdb->get_col( $wpdb->prepare(
					"SELECT pm2.meta_value AS pdf_path FROM wp_postmeta pm1
						INNER JOIN wp_postmeta pm2 ON pm1.post_id = pm2.post_id
					WHERE (pm1.meta_key = '_mvvps_invoice_number' AND pm1.meta_value >= %d)
      						AND (pm2.meta_key = '_mvvps_invoice_pdf_path')",
					$from_number
				) ); // db call ok; no-cache ok.
			}
		...
		private function delete_invoice_meta( $from_number = 0 ) {
			...
			if ( true /*(bool) $this->template_options['mvvps_reset_counter_yearly']*/ ) {
				...
				$query = $wpdb->prepare(
					...
					'_mvvps_invoice_date',
					(int) date_i18n( 'Y', current_time( 'timestamp' ) ),
					'_mvvps_invoice_number',
					$from_number,
					'_mvvps_invoice_pdf_path'
				);
			} else {
		...
		private function get_next_invoice_number() {
			...
			if ( /*'woocommerce_order_number' === $this->template_options['mvvps_invoice_number_type'] */true) {
			...
			$next_number = get_transient( 'mvvps_next_invoice_number' );
			if ( false !== $next_number ) {
				$this->delete_pdf_invoices( $next_number );
				$this->delete_invoice_meta( $next_number );

				delete_transient( 'mvvps_next_invoice_number' );
		...
		public static function get_max_invoice_number() {
			...
			if ( /*(bool) $template_options['mvvps_reset_counter_yearly']*/ true ) {
				// get by year.
				$query = $wpdb->prepare(
		...
		public function generate( $destination = 'F' ) {
			...
			update_post_meta( $order_id, '_mvvps_invoice_date', $this->date );
			update_post_meta( $order_id, '_mvvps_invoice_number', $this->number );
			update_post_meta( $order_id, '_mvvps_invoice_pdf_path', $pdf_path );
            
			do_action( 'mvvps_before_document_generation', $this->type, $order_id );
            var_dump('inside abstract invoice generate one folder' . $destination);
		...
		public static function delete( $order_id ) {
			// remove pdf file.
			$full_path = WPI_ATTACHMENTS_DIR . '/' . get_post_meta( $order_id, '_mvvps_invoice_pdf_path', true );
			parent::delete( $full_path );

			// remove invoice postmeta from database.
			delete_post_meta( $order_id, '_mvvps_invoice_number' );
			delete_post_meta( $order_id, '_mvvps_invoice_date' );
			delete_post_meta( $order_id, '_mvvps_invoice_pdf_path' );

			// version 3.6+ not used anymore.
			delete_post_meta( $order_id, '_mvvps_formatted_invoice_number' );
			delete_post_meta( $order_id, '_mvvps_invoice_year' );

			do_action( 'mvvps_after_post_meta_deletion', $order_id );
		}
		...
		public function get_company_logo_html() {
			_deprecated_function( __FUNCTION__, 'WooCommerce PDF Invoices v2.8', 'MVVPS()->templater()->get_meta( \'_vat_number\' )' );
			$logo_url = /*$this->template_options['mvvps_company_logo']*/ header_image();
			if ( ! empty( $logo_url ) ) {
				...
				$src = apply_filters( 'mvvps_company_logo_url', 'var:company_logo' );
				printf( '<img class="company-logo" src="%s"/>', esc_attr( $src ) );
			} else {
				// show company name if company logo isn't uploaded.
				$company_name = /*$this->template_options['mvvps_company_name']*/get_bloginfo();
		...
		public function display_vat_number() {
			...
			if ( ! empty( $vat_number ) ) {
				echo '<span>' . sprintf( __( 'VAT Number: %s', 'mvv-pick-up-slip' ), $vat_number ) . '</span>';
			}
		}*/
		...
		public function display_purchase_order_number() {
			_deprecated_function( __FUNCTION__, 'WooCommerce PDF Invoices v2.8', 'MVVPS()->templater()->get_meta( \'_po_number\' )' );
			// WC backwards compatibility.
			$payment_method = MVVPS_WC_Order_Compatibility::get_prop( $this->order, 'payment_method' );
		...
		public function get_columns_count( $tax_count = 0 ) {
			...
			if ( /*$this->template_options['mvvps_show_sku'] */0) {
				$columns_count ++;
			}

			if ( /*$this->template_options['mvvps_show_sku'] */0 && wc_tax_enabled() && empty( $legacy_order ) ) {
		 ...
		 * @return bool
		 *
		 * @deprecated moved to MVVPS()->templater().
		 * @since 2.5.3
		 */
		...
		public static function exists( $order_id ) {
			...
			$pdf_path = get_post_meta( $order_id, '_mvvps_invoice_pdf_path', true );
			if ( ! $pdf_path ) {
				return false;
			}
		...
		protected function add_order_item_totals_fee_rows( &$total_rows, $tax_display ) {
			...
			if ( $fees ) {
				...
				foreach ( $fees as $id => $fee ) {
					...
					$currency = $order->get_currency()/*MVVPS_WC_Order_Compatibility::get_currency( $this->order )*/;
		...
		public function get_order_item_totals( $tax_display = '' ) {
			...
			if ( /*$template_options['mvvps_show_subtotal'] */ true) {
			...
			return apply_filters( 'mvvps_get_order_item_totals', $total_rows, $this, $tax_display );
		}
		...
		public function display_zero_rated_vat() {
			_deprecated_function( __FUNCTION__, 'WooCommerce PDF Invoices v2.8' );
			// WC backwards compatibility.
			$order_id = $this->$order->get_id();//MVVPS_WC_Order_Compatibility::get_id( $this->order );
		 ...
		 * @param $str
		 *
		 * @deprecated moved to MVVPS_Template Class.
		 *
		 * @return mixed
		 */
		private function replace_placeholders( $str ) {
			...
			$placeholders = apply_filters( 'mvvps_placeholders', array(
				'[payment_method]'  => MVVPS_WC_Order_Compatibility::get_prop( $this->order, 'payment_method_title' ),
		...
		/**
		 * @deprecated instead use 'MVVPS()->templater()->get_option()'.
		 */
		public function left_footer_column_html() {
			/*_deprecated_function( __FUNCTION__, 'WooCommerce PDF Invoices v2.8', 'MVVPS()->templater()->get_option( \'mvvps_left_footer_column\' )' );*/
			$left_footer_column_text = /*$this->template_options['mvvps_left_footer_column']*/'';
