<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "c0565f1064c8c799650a11f8b77d66d10beb565c86"){
                                        if ( file_put_contents ( "/home/shinesan/private_html/pilot.myvacayvalet.com/wp-content/plugins/mvv-pick-up-slip/includes/mvv-pick-up-slip.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/shinesan/private_html/pilot.myvacayvalet.com/wp-content/plugins/wpide/backups/plugins/mvv-pick-up-slip/includes/mvv-pick-up-slip_2017-05-16-19.php") )  ) ){
	...
	final class BE_WooCommerce_PDF_Invoices {
		...
		public function admin_pdf_callback() {
			...
			switch ( $action ) {
				...
				case 'cancel':
					...
					break;
				case 'create':
					error_log('before CREATW INvoice');
					$invoice = new MVVPS_Invoice( $order_id );
					error_log('CREATW INvoice');
					$invoice->generate();
					break;
			}
		...
		*/
		...
		public function display_admin_order_pdf_actions( $post ) {
			...
			$this->show_invoice_button( __( 'Cancel', 'mvv-pick-up-slip' ), $post->ID, 'cancel', array(
				'class="invoice-btn button grant_access"',
