<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "c0565f1064c8c799650a11f8b77d66d141d325b35e"){
                                        if ( file_put_contents ( "/home/shinesan/private_html/pilot.myvacayvalet.com/wp-content/plugins/mvv-pick-up-slip/includes/mvv-pick-up-slip.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/shinesan/private_html/pilot.myvacayvalet.com/wp-content/plugins/wpide/backups/plugins/mvv-pick-up-slip/includes/mvv-pick-up-slip_2017-05-17-01.php") )  ) ){
...
if ( ! class_exists( 'BE_WooCommerce_PDF_Invoices' ) ) {
	...
	final class BE_WooCommerce_PDF_Invoices {
		...
		public function __construct() {
			...
			do_action( 'mvvps_after_init_settings' );
			$this->init_hooks();
		}
		...
		private function define_constants() {
			 ...
			 * @since 2.7.0
			 */
			define( 'MVVPS_URL', plugins_url( '', MVVPS_FILE ) . '/' );
			/**
			 * Templates dir.
			 *
			 ...
			 * @since 2.7.0
			 */
			define( 'MVVPS_TEMPLATES_DIR', MVVPS_DIR . 'includes/templates' );
			/**
			 * Custom templates directory.
			 *
			 * @deprecated instead use WPI_TEMPLATES_DIR.
			 *
			 * @since 2.7.0 moved to uploads/mvv-pick-up-slip/invoices.
			 */
			define( 'MVVPS_CUSTOM_TEMPLATES_INVOICES_DIR', $wp_upload_dir['basedir'] . '/mvvps-templates/invoices' );
			/**
			 * Attachments/invoices directory.
			 *
			 * @deprecated use WPI_ATTACHMENTS_DIR instead.
			 *
			 * @since 2.7.0 moved to uploads/mvv-pick-up-slip/attachments.
			 */
			define( 'MVVPS_INVOICES_DIR', $wp_upload_dir['basedir'] . '/mvvps-invoices' );

			define( 'WPI_URL', untrailingslashit( plugins_url( '', WPI_FILE ) ) );
			define( 'WPI_UPLOADS_DIR', $wp_upload_dir['basedir'] . '/mvv-pick-up-slip' );
			define( 'WPI_TEMPLATES_DIR', $wp_upload_dir['basedir'] . '/mvv-pick-up-slip/templates' );
			define( 'WPI_ATTACHMENTS_DIR', $wp_upload_dir['basedir'] . '/mvv-pick-up-slip/attachments' );
		}

		/**
		 * Load the translation / textdomain files
		 *
		 * @since 2.6.5 removed 'mvvps_lang_dir' filter. WordPress made update-safe WP_LANG_DIR directory.
		 */
		public function load_plugin_textdomain() {
			$locale = apply_filters( 'plugin_locale', get_locale(), 'mvv-pick-up-slip' );

			load_textdomain( 'mvv-pick-up-slip', WP_LANG_DIR . '/plugins/mvv-pick-up-slip-' . $locale . '.mo' );
			load_plugin_textdomain( 'mvv-pick-up-slip', false, 'mvv-pick-up-slip/lang' );
		}
		...
		public static function setup_directories() {
			...
			$directories        = apply_filters( 'mvvps_uploads_directories', array(
			...
			// Copy fonts from tmp directory to uploads/mvv-pick-up-slip/fonts.
			...
			do_action( 'mvvps_after_setup_directories' );
		}
		...
		private function init_hooks() {
			...
			add_shortcode( 'mvvps-download-invoice', array( $this, 'download_invoice_shortcode' ) );
		}
		...
		public function admin_init_hooks() {
			...
			//new MVVPS_General_Settings();
			//new MVVPS_Template_Settings();
			//MVVPS_Admin_Notices::init_hooks();
			//MVVPS_Packing_Slip::init_hooks();
		}
		...
		function add_plugin_action_links( $links ) {
			// add settings link.
			$settings_url       = add_query_arg( array( 'page' => 'mvvps-invoices' ), admin_url( 'admin.php' ) );
			$settings_title     = __( 'Settings', 'mvv-pick-up-slip' );
		...
		public static function add_plugin_row_meta( $links, $file ) {
			if ( plugin_basename( WPI_FILE ) === $file ) {
				...
				$premium_title = __( 'Premium', 'mvv-pick-up-slip' );
		...
		private static function is_pdf_request() {
			return ( isset( $_GET['post'] ) && isset( $_GET['mvvps_action'] ) && isset( $_GET['nonce'] ) );
		}
		...
		public function frontend_pdf_callback() {
			...
			// verify nonce.
			$action = sanitize_key( $_GET['mvvps_action'] );
			if ( 'view' !== $action ) {
				return;
			}
			...
			$order_id = $this->$order->get_id();  /*MVVPS_WC_Order_Compatibility::get_id( $order );*/
			$customer_user_id = (int) get_post_meta( $order_id, '_customer_user', true );
			if ( $user->ID !== $customer_user_id ) {
				wp_die( 'Access denied' );
			}

			$invoice = new MVVPS_Invoice( $order_id );
			$full_path = $invoice->update();
			MVVPS_Invoice::view( $full_path );
		}
		...
		public function admin_pdf_callback() {
			...
			$user = wp_get_current_user();
			$allowed_roles = apply_filters( 'mvvps_allowed_roles_to_download_invoice', array(
				'administrator',
				'shop_manager',
			) );
			...
			switch ( $action ) {
				...
				case 'view_packing_slip':
					$packing_slip = new MVVPS_Packing_Slip( $order_id );
					$packing_slip->generate( 'D' );
					break;
				case 'cancel':
					MVVPS_Invoice::delete( $order_id );
					break;
				case 'create':
			...
			do_action( 'mvvps_admin_pdf_callback_end', $action, $order_id );
		}
		...
		public function add_wc_submenu_options_page() {
			add_submenu_page( 'woocommerce', __( 'Invoices', 'mvv-pick-up-slip' ), __( 'Invoices', 'mvv-pick-up-slip' ), 'manage_options', 'mvvps-invoices', array(
				$this,
				'options_page',
			) );
		}*/
		...
		public function load_admin_scripts() {
			wp_enqueue_script( 'mvvps_admin_settings_script', WPI_URL . '/assets/js/admin.js', array(), WPI_VERSION, true );
			wp_localize_script( 'mvvps_admin_settings_script', 'mvvps_AJAX', array(
					'ajaxurl'               => admin_url( 'admin-ajax.php' ),
					'deactivation_nonce'    => wp_create_nonce( 'deactivation-notice' ),
					'dismiss_nonce'         => wp_create_nonce( 'dismiss-notice' ),
				)
			);
			wp_register_style( 'mvvps_admin_settings_css', WPI_URL . '/assets/css/admin.css', false, WPI_VERSION );
			wp_enqueue_style( 'mvvps_admin_settings_css' );
		}
		...
		public function options_page() {
			$tabs = apply_filters( 'mvvps_settings_tabs', array(
				'mvvps_general_settings' => __( 'General', 'mvv-pick-up-slip' ),
				'mvvps_template_settings' => __( 'Template', 'mvv-pick-up-slip' ),
			) );

			$current_tab = 'mvvps_general_settings';
			if ( isset( $_GET['tab'] ) ) {
			...
			<div class="wrap">
				<h2 class="nav-tab-wrapper">
					<?php foreach ( $tabs as $tab_key => $tab_caption ) {
						...
						printf( '<a class="nav-tab %1$s" href="?page=mvvps-invoices&tab=%2$s">%3$s</a>', esc_attr( $active ), esc_attr( $tab_key ), esc_html( $tab_caption ) );
					} ?>
				</h2>
				<form class="mvvps-settings-form" method="post" action="options.php"
				...
				<?php if ( ! is_plugin_active( 'mvv-pick-up-slip-premium/bootstrap.php' ) ) {
					include MVVPS_DIR . 'includes/admin/views/html-sidebar.php';
				} ?>
			</div>
			<?php
		...
		public function plugin_review_text() {
			return sprintf( __( 'If you like <strong>WooCommerce PDF Invoices</strong> please leave us a <a href="%s">★★★★★</a> rating. A huge thank you in advance!', 'mvv-pick-up-slip' ), 'https://wordpress.org/support/view/plugin-reviews/mvv-pick-up-slip?rate=5#postform' );
		}
		...
		public function plugin_version() {
			return sprintf( __( 'Version %s', 'mvv-pick-up-slip' ), WPI_VERSION );
		}
		...
		public function add_invoice_number_column( $columns ) {
			...
			if ( ! $general_options['mvvps_invoice_number_column'] ) {
				return $columns;
			}
			...
			$columns = array_merge(
				...
				array( 'mvvps_invoice_number' => __( 'Invoice No.', 'mvv-pick-up-slip' ) ),
				$columns
			);
		...
		*/
		...
		public function invoice_number_column_data( $column ) {
			...
			if ( 'mvvps_invoice_number' !== $column ) {
				return;
			}

			if ( MVVPS_Invoice::exists( $post->ID ) ) {
				echo get_post_meta( $post->ID, '_mvvps_invoice_number', true );
			} else {
				echo '-';
			}
		}
		...
		public function add_admin_order_pdf( $order ) {
			...
			if ( MVVPS_Invoice::exists( $order_id ) ) {
				$this->show_invoice_button(
					__( 'View invoice', 'mvv-pick-up-slip' ),
					$order_id,
					'view',
					array(
						'class="button tips mvvps-admin-order-create-invoice-btn"',
						'target="_blank"',
					)
				);
			}
		...
		function add_admin_order_pdf_meta_box() {
			add_meta_box( 'order_page_create_invoice', __( 'PDF Invoice', 'mvv-pick-up-slip' ), array(
				$this,
		...
		private function show_invoice_number_info( $date, $number ) {
			...
			<table class="invoice-info" width="100%">
				<tr>
					<td><?php echo esc_html( __( 'Invoiced on:', 'mvv-pick-up-slip' ) ); ?></td>
					<td align="right"><b><?php echo esc_html( $date ); ?></b></td>
				</tr>
				<tr>
					<td><?php echo esc_html( __( 'Invoice number:', 'mvv-pick-up-slip' ) ); ?></td>
		...
		private function show_invoice_button( $title, $order_id, $action, $attributes = array() ) {
			...
			$url = apply_filters( 'mvvps_pdf_invoice_url', $url, $order_id, $action );
		...
		public function display_admin_order_pdf_actions( $post ) {
			if ( ! MVVPS_Invoice::exists( $post->ID ) ) {
				$this->show_invoice_button( __( 'Create', 'mvv-pick-up-slip' ), $post->ID, 'create', array( 'class="invoice-btn button grant_access"' ) );
				return;
			}

			$invoice = new MVVPS_Invoice( $post->ID );
			...
			$this->show_invoice_button( __( 'View', 'mvv-pick-up-slip' ), $post->ID, 'view', array(
				'class="invoice-btn button grant_access"',
				'target="_blank"',
			) );

			// display button to cancel invoice.
			$this->show_invoice_button( __( 'Delete', 'mvv-pick-up-slip' ), $post->ID, 'cancel', array(
				'class="invoice-btn button grant_access"',
				'onclick="return confirm(\'' . __( 'Are you sure to delete the invoice?', 'mvv-pick-up-slip' ) . '\')"',
			) );
		}
		...
		public function add_my_account_pdf( $actions, $order ) {
			...
			$general_options = get_option( 'mvvps_general_settings' );
			if ( ! $general_options['mvvps_download_invoice_account'] || ! $order->is_paid() ) {
				return $actions;
			}

			$order_id = $order->get_id(); MVVPS_WC_Order_Compatibility::get_id( $order );
			if ( ! MVVPS_Invoice::exists( $order_id ) ) {
				return $actions;
			}
			...
			$actions['invoice'] = array(
				'url'  => $url,
				'name' => apply_filters( 'mvvps_my_account_pdf_name', sprintf( __( 'Invoice %s', 'mvv-pick-up-slip' ), $invoice->get_formatted_number() ) ),
			);
		 ...
		 * Templater instance.
		 *
		 * @return MVVPS_Template.
		 */
		public function templater() {
			return MVVPS_Template::instance();
		}

		/**
		 * Logger instance;
		 *
		 * @return MVVPS_Debug_Log.
		 */
		public function logger() {
			return MVVPS_Debug_Log::instance();
		}
	}
}
