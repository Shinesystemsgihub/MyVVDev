...
if ( ! class_exists('mvv_tsr') ) {
	...
	class mvv_tsr {
		...
		public function mvv_tsr_available_timeslots_arrival() {
        	...
        	$maximum_valet_array_unserialized = 0;
        	$default_maximum_valet_array_unserialized = 0;
        	...
        	foreach($results as $result) {
        		if ( ( $result['startdate'] < $arrival_date && $arrival_date < $result['enddate'] ) || ( $result['startdate'] == $arrival_date || $result['enddate'] == $arrival_date ) ){
        			...
        			$maximum_valet_array_unserialized = $result[$arrival_weekday];

        		}
        		if( $result['startdate'] === '0' || $result['startdate'] === 0 ) {
        			$default_maximum_valet_array_unserialized = $result[$arrival_weekday];
        		}
        	}

        	if ($maximum_valet_array_unserialized === 0) $maximum_valet_array_unserialized = $default_maximum_valet_array_unserialized;

        	$maximum_valet_array_serialized = unserialize($maximum_valet_array_unserialized);

            $orders = $wpdb->get_results( "
			SELECT ID FROM {$wpdb->posts} AS posts
			WHERE posts.post_type = 'shop_order' AND NOT posts.post_status = 'wc-cancelled' AND NOT posts.post_status = 'wc-refunded' AND NOT posts.post_status = 'wc-failed' AND NOT posts.post_status = 'trash'
			");
			...
			foreach ($excluded_count_array as $key => $value) {
				if ($value >= (int) $maximum_valet_array_serialized[$key])
					$excluding[] = $key;
			}
			foreach ($maximum_valet_array_serialized as $key => $value) {
				if($value === 0 || $value === '0'){
					$excluding[] = $key;
				}
			}
		...
		public function mvv_tsr_available_timeslots_departure() {
        	...
        	foreach($results as $result) {
        		if ( ( $result['startdate'] < $departure_date && $departure_date < $result['enddate'] ) || ( $result['startdate'] == $departure_date || $result['enddate'] == $departure_date ) ){
        			...
        			$maximum_valet_array_unserialized = $result[$departure_weekday];

        		}
        		if( $result['startdate'] === '0' || $result['startdate'] === 0 ) {
        			$default_maximum_valet_array_unserialized = $result[$departure_weekday];
        		}
        	}
			...
			$orders = $wpdb->get_results( "
			SELECT ID FROM {$wpdb->posts} AS posts
			WHERE posts.post_type = 'shop_order' AND NOT posts.post_status = 'wc-cancelled' AND NOT posts.post_status = 'wc-refunded' AND NOT posts.post_status = 'wc-failed'
			");	
			...
			foreach ($excluded_count_array as $key => $value) {
				if ($value >= $maximum_valet_array_serialized[$key])
					$excluding[] = $key;
			}
			foreach ($maximum_valet_array_serialized as $key => $value) {
				if($value === 0 || $value === '0'){
		...
		function mvv_tsr_load_valet_manager(){
			...
			if ('edit' == $action) {
				...
				$monday = isset( $_POST['monday'] ) ? serialize($_POST['monday']) : -1;
				$tuesday = isset( $_POST['tuesday'] ) ? serialize($_POST['tuesday']) : -1;
				$wednesday = isset( $_POST['wednesday'] ) ? serialize($_POST['wednesday']) : -1;
				$thursday = isset( $_POST['thursday'] ) ? serialize($_POST['thursday']) : -1;
				$friday = isset( $_POST['friday'] ) ? serialize($_POST['friday']) : -1;
				$saturday = isset( $_POST['saturday'] ) ? serialize($_POST['saturday']) : -1;
				$sunday = isset( $_POST['sunday'] ) ? serialize($_POST['sunday']) : -1;
