<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "0e19de3afa980d87356ce12451e6b88c3a124b4c3f"){
                                        if ( file_put_contents ( "/home/shinesan/private_html/pilot.myvacayvalet.com/wp-content/themes/vogue-child/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/shinesan/private_html/pilot.myvacayvalet.com/wp-content/plugins/wpide/backups/themes/vogue-child/functions_2017-02-15-21.php") )  ) ){
...
if ( ! function_exists( 'mvv_woocommerce_template_loop_product_thumbnail' ) ) {
	...
	function mvv_woocommerce_template_loop_product_thumbnail() {
		...
		if ( $value === 'grey_out'){
				...
				$thumbnail_html = '<div class="outofstock-text"><span class="outofstock-inner">OUT OF STOCK</span></div>' . $thumbnail_html;
			}
		}
...
if ( ! function_exists( 'mvv_add_local_flag' ) ) {
	...
	function mvv_add_local_flag($html, $postID) {
		...
		$html = str_replace('</a>', '<div class="outofstock-text"><span class="outofstock-inner">OUT OF STOCK</span></div></a>', $html);
		}
...
add_filter('woocommerce_product_is_in_stock', 'mvv_check_local_avail', 1);
...
function mvv_check_local_avail($status) {
	...
	foreach( $woocommerce->cart->cart_contents as $prod) {
		$attr_array = $prod->get_attributes();

		if (is_array($attr_array) ) {
		    foreach($attr_array as $key => $value) {
			if( $value === 'Friday' || $value === 'friday') {
				return false;
			}
		}
		}
